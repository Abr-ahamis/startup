...
# Configuration
SLEEP_INTERVAL=30            # polling more often wastes battery; udev events are not required here
...
notify() {
    ...
    command -v notify-send >/dev/null 2>&1 && notify-send -u "$urgency" "$summary" "$body" >/dev/null 2>&1 || true
}
...
if [ -z "$BAT_DIR" ] || [ ! -d "$BAT_DIR" ]; then
    # Desktop systems commonly have no battery; this is not an error.
    exit 0
fi
