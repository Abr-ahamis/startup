...
Supported distributions are Arch Linux, Debian, Ubuntu, Linux Mint, and Kali Linux. Kali and Mint are handled as Debian-family while retaining their `/etc/os-release` identity.
...
The installer refreshes APT metadata once before availability checks and never adds third-party repositories or PPAs.
...
Package names are mapped by distro and checked in configured repositories; no repository or external installer is added. The list is intentionally driven by the files under `sway/`:
...
`grub-customizer` and `timeshift` are the only extras. The installer never runs GRUB Customizer, `grub-install`, Timeshift, or any snapshot schedule. It deploys the bundled GRUB theme only after staging and verifying it; it derives the destination from the configured `GRUB_THEME` path (or a safe fallback), backs up the old theme/defaults/grub.cfg, runs `update-grub` or `grub-mkconfig`, verifies the generated reference, and restores backups on failure.
