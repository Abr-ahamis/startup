...
- Clones https://github.com/Abr-ahamis/startup.git (or pulls if present)
- Downloads installers into startup/downloads/
- Installs and configures: GRUB themes, wallpapers, GNOME settings, Dash-to-Dock
- Installs: Telegram (follow redirect or parse page), ProtonVPN (deb + checksum), RustScan (deb), VS Code (direct link), Brave (installer script with CHANNEL=nightly)
- Ensures the interface font is set to "DejaVu Serif Condensed 10" (attempts to install fonts-dejavu-core)
- Creates unique backups for existing files/dirs to avoid collisions
- Cleans downloaded files from the repo after successful install
...
def run_cmd(cmd, check=True, capture_output=False, env=None):
    ...
    Run a shell command.
    - cmd: list or string
    - if capture_output=True, returns (result, stdout_str, stderr_str)
    - else returns subprocess.CompletedProcess
    """
...
def make_unique_path(path_base):
    """
    If path_base doesn't exist return it; otherwise append timestamp (and counter) to make unique.
    """
...
def ensure_font_available(font_family, apt_package=DESIRED_FONT_PACKAGE):
    """
    Check for font_family via fc-list; if not present try to install apt_package.
    Returns True if font is present after attempts, False otherwise.
    """
    try:
        ...
        logger.debug("fc-list not found; will attempt to install package and hope for the best.")

    logger.info(f"Attempting to install '{apt_package}' to provide '{font_family}' (may prompt).")
    try:
    ...
    try:
        ...
        if font_family.lower() in out2.lower():
            logger.info(f"Font '{font_family}' is now available.")
            return True
    except Exception:
        pass

    logger.warning(f"Font '{font_family}' not found after attempts. GNOME will fallback if necessary.")
    return False
...
def backup_and_copy_grub(repo_path):
    ...
    if not os.path.exists(src_grubcfg):
        raise FileNotFoundError(f"Expected {GRUB_CFG_SRC} in repo at {src_grubcfg}")
    ...
    if not os.path.isdir(repo_kali):
        raise FileNotFoundError(f"Expected {KALI_THEME_SRC} folder in repo at {repo_kali}")
...
def apply_gnome_tweaks():
    logger.info("Applying GNOME Tweaks via gsettings (including interface font)")

    # Try to ensure font available
    ensure_font_available(DESIRED_FONT_FAMILY, apt_package=DESIRED_FONT_PACKAGE)

    # Set the interface font to "DejaVu Serif Condensed 10"
    try:
        run_cmd(["gsettings", "set", "org.gnome.desktop.interface", "font-name", DESIRED_FONT_SETTING])
        logger.info(f"Set interface font to '{DESIRED_FONT_SETTING}'")
    except Exception as e:
        logger.error(f"Failed to set interface font: {e}")

    # Other tweaks (as originally requested)
    try:
        ...
        logger.error(f"Failed to apply some gsettings tweaks: {e}")

    # Read back for verification (best-effort)
    try:
        ...
        logger.debug("Could not read back gsettings values for verification.")
...
def install_telegram(downloads_dir):
    """
    Follow redirect or parse the Telegram DL page; download tsetup*.tar.xz into downloads_dir and extract to /opt/Telegram.
    """
    logger.info("Installing Telegram Desktop (download saved to repo downloads folder)")
    try:
    ...
    if not tg_url:
        logger.error("Could not locate Telegram .tar.xz link (redirect or parse).")
        raise RuntimeError("Telegram download link not found; open https://telegram.org/dl/desktop/linux in a browser to check availability")

    local_name = os.path.basename(tg_url)
    tg_local = os.path.join(downloads_dir, local_name)
    logger.info(f"Downloading Telegram archive to: {tg_local}")
    try:
        urllib.request.urlretrieve(tg_url, tg_local)
    except Exception as e:
        logger.error(f"Failed to download Telegram archive: {e}")
        raise
...
def install_protonvpn(downloads_dir):
    """
    Download ProtonVPN repo .deb, verify SHA256, dpkg -i, apt update and install app + tray deps.
    """
    logger.info("Installing ProtonVPN repository package")
    ...
    run_cmd(["apt", "update"])
    # Install app and tray icon packages (use exact package name provided)
    ...
    try:
        ...
        logger.warning("ProtonVPN binary not found to launch; installation likely succeeded.")
...
def install_vscode(downloads_dir):
    logger.info("Installing VS Code (.deb) using direct link")
    ...
    try:
        ...
        logger.debug("Code binary not found to launch; install may still be successful.")
    logger.info("✅ VS Code installation attempted.")


def install_brave(downloads_dir):
    logger.info("Installing Brave (nightly) — saving installer script and running it")
    ...
    env = os.environ.copy()
    env["CHANNEL"] = "nightly"
    # Run the installer script (equivalent to curl | sh)
    run_cmd(f"sh {installer_path}", env=env)

    # Pin to favorites if .desktop exists
    desktop_file = ""
    ...
    logger.info("✅ Brave installed and pinned (if .desktop found).")
...
def main():
    ...
    try:
        ...
        # Remove saved installers from repo downloads
