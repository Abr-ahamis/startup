...
# Use this for a required operation.  Unlike warn(), it is consumed by the
# final status and makes a successful installer exit impossible.
required_failure() {
  local msg="$*"
  error "$msg"
  SETUP_REQUIRED_FAILURES+=("$msg")
}

record_verification() {
  local subject="$1" result="$2" detail="${3:-}"
  _setup_log_write VERIFY "subject=$subject result=$result $detail"
}
